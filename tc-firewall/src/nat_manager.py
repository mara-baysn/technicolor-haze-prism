...
def add_snat(private_ip: str, public_ip: str, comment: Optional[str] = None) -> NATEntry:
    ...
    _run_tc(cmd)
    logger.info(f"Added SNAT rule {rule_id}: {private_ip} -> {public_ip}")

    # Get handle and check hw offload
